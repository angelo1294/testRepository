...
def getUrl(endpoint):
    urlDict = {
        ...
        'topology':          '/velocity/api/topology/v10/topology',
        'topologies':        '/velocity/api/topology/v10/topologies',
...
def getTopologies(velo='', searchString='', offset=0):
    topologyList = []
    url = 'https://' + velo + '.spirenteng.com' + getUrl('topologies') + '?searchString=' + searchString + '&limit=100&' + 'offset=' + str(offset)
    rq = requests.get(url, verify=False, auth=('spirent', 'spirent'))
...
def putUser(velo='', userId='', body='', sysUser='spirent', sysPass='spirent'):
    url = 'https://' + velo + '.spirenteng.com' + getUrl('user') + '/' + userId
    rq = requests.put(url, data=json.dumps(body), verify=False, auth=(sysUser, sysPass))
    if 'errorId' in rq.text:
        print('Error encountered when editing user: ' + userId + rq.text)
...
def getUsers(velo='', limit=200, username='spirent', password='spirent', role=''):
    offset = 0
    response = {}
    while (True):
    ###Filter by role if given
        if role:
            url = 'https://' + velo + '.spirenteng.com' + getUrl('user') + 's' + '/' + '?limit=' + str(
                limit) + '&offset=' + str(offset) + '&filter=role::' + role
        else:
            url = 'https://' + velo + '.spirenteng.com' + getUrl('user') + 's' + '/' + '?limit=' + str(
        ...
        if 'errorId' not in rq.text:
            ...
            for index in range(0, int(rq['count'])):
                response[rq['profiles'][index]['login']] = rq['profiles'][index]
            if (int(rq['offset']) + limit) > int(rq['total']):
                return response
            else:
                offset = offset + limit
        else:
...
def getUser(velo='',username='spirent', password='spirent', loginName=''):
    response = {}
    url = 'https://' + velo + '.spirenteng.com' + getUrl('user') + 's' + '?' + 'filter=login::' + loginName
    rq = requests.get(url, verify=False, auth=(username, password))
    if 'errorId' not in rq.text:
        rq = json.loads(rq.text)
        return rq['profiles'][0]['id']
    else:
        print('Error encountered when fetching user list')
        return response
